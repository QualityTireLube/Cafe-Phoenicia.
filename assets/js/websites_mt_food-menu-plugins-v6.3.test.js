...
function newTemplateCartSidebar() {
    ...
    if ($(window).width() > 767) {
        ...
        if (cartItems.length > 0) {
            ...
            $('.food-menu-page').append('<div id="sh-shopping-cart-sidebar" class="container" style="position:fixed;top:'+marginTop+'px;right:0;background-color:#fff;padding:15px;text-align:center;color:#000;text-transform:uppercase;border: 1px solid #eee;max-width:'+maxWidth+'"><div class="col-md-12 col-sm-12"><h3 style="font-family:Arial, Helvetica, sans-serif;" class="pull-left">Your Order:</h3><button style="background: none;border: none;color:#000;text-decoration:underline;" class="pull-right sh-toggle-cart">Hide Cart</button></div><div class="sh-sidebar-items" style="max-height: 170px;overflow: auto;margin: 4px 4px;padding: 4px;"></div></div>');
...
function ShCreateOrUpdateOrder(url) {
    var shModal = '<div style="z-index:9999;" class="modal fade" id="testShCartMessage" role="dialog"> <div class="modal-dialog"><div class="modal-content"> <div class="modal-header"> <button type="button" class="close" data-dismiss="modal">&times;</button> <h4 style="color:#000;font-size: 1.3em;font-family: Arial, Helvetica, sans-serif;" class="modal-title">Note</h4> </div> <div class="modal-body text-center"><p style="font-size: 1.2em;">Checkouts cannot be completed on test sites.</p></div><div style="text-align: center !important;" class="modal-footer "> <button style="font-size: 14px;font-family: Arial, Helvetica, sans-serif;" type="button" class="btn btn-success" data-dismiss="modal">Close</button> </div> </div> </div> </div>';
        $('.food-menu-page').append(shModal);

        $('#testShCartMessage').modal('show');
}

function newTemplateShProceedToCheckout(){
  $("body").on('click', '.sh-checkout', function(){
      ShCreateOrUpdateOrder();
  });
}
...
function initializeShCart(url) {
  if ($('.food-menu-nav').length) { // check is it a new template
      ...
      newTemplateShProceedToCheckout();
  }
}
